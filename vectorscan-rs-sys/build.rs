...
fn build_vectorscan(manifest_dir: &Path, out_dir: &Path, version: &str, is_windows_gnu: bool) {
    ...
    let vectorscan_src_dir = out_dir.join(format!("vectorscan-{version}"));

    // 5.4.11: patch generated with `diff -ruN`, applied with -p1
    // 5.4.12: patch generated with `git diff --no-index`, applied with -p2
    let (patchfile, patch_strip) = if is_windows_gnu {
        (manifest_dir.join("vectorscan-5.4.12.patch"), "2")
    } else {
        (manifest_dir.join("vectorscan.patch"), "1")
    };

    // Extract release tarball, stripping the first path component so that different tarball
    // prefixes (e.g. `vectorscan-vectorscan-5.4.11/` vs `vectorscan-{commit}/`) all extract
    // to the same consistent `vectorscan-{version}/` directory.
    {
...
fn main() {
    ...
    let version = if is_windows_gnu { "5.4.12" } else { "5.4.11" };
    ...
    println!("cargo:rerun-if-changed=vectorscan-5.4.12.patch");
    ...
    if is_windows_msvc {
        ...
        if let Some(lib_dir) = std::env::var_os("VECTORSCAN_LIB_DIR") {
            ...
            build_vectorscan(&manifest_dir, &out_dir, version, is_windows_gnu);
        }
